...
const generateConfig = ( options ) =>
{
    ...
    const distFiles       = path.resolve( pwd, options.components );
    const srcFiles        = path.resolve( pwd, `${options.componentsSr}/src` );
    const jsonDefaults    = path.resolve( pwd, options.showcasePropsJson );
    const componentStyles =
        path.resolve( pwd, `${options.components}/dist/styles.css` );
